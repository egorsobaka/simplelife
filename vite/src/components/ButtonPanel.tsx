    ...
    <div style={getPanelStyle()}>
      {buttons.map(button => (
        <ButtonItem
          key={button.type}
          button={button}
          isActive={activePanel === button.type}
...
export const defaultButtons: ButtonConfig[] = [
  { type: 'inventory', icon: '🎒', label: 'Инвентарь' },
  { type: 'leaderboard', icon: '🏆', label: 'Лидерборд' },
  { type: 'crafting', icon: '⚒️', label: 'Крафтинг' },
  { type: 'chat', icon: '💬', label: 'Чат' },
  { type: 'settings', icon: '⚙️', label: 'Настройки' },
];