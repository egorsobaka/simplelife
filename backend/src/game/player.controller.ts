...
export class PlayerController {
  ...
  async getPlayerInventory(@Param('telegramId') telegramId: string) {
    const player = this.gameService.getPlayerByTelegramId(telegramId);
    if (!player) {
      return { error: 'Player not found' };
    }
   ...
   */
  @Post(':telegramId/inventory/add')
  async addItemToInventory(
    @Param('telegramId') telegramId: string,
    @Body() itemData: { itemType: string; count?: number }
  ) {
    const player = this.gameService.getPlayerByTelegramId(telegramId);
    if (!player) {
      return { error: 'Player not found' };
    ...
    for (let i = 0; i < count; i++) {
      player.inventory.push(itemData.itemType);
    }
    ...
    return {
      success: true,
      message: `Added ${count} ${itemData.itemType}(s) to inventory`,
      inventory: this.gameService.getPlayerInventoryByTelegramId(telegramId)
    };
  }
   ...
   */
  @Post(':telegramId/inventory/remove')
  async removeItemFromInventory(
    ...
    for (let i = 0; i < count; i++) {
      const index = player.inventory.indexOf(itemData.itemType);
      if (index !== -1) {
    ...
    return {
      success: true,
      message: `Removed ${removed} ${itemData.itemType}(s) from inventory`,
      removed,
      requested: count,
      inventory: this.gameService.getPlayerInventoryByTelegramId(telegramId)
    };
   ...
   */
  @Delete(':telegramId/inventory')
  async clearInventory(@Param('telegramId') telegramId: string) {
    const player = this.gameService.getPlayerByTelegramId(telegramId);
    if (!player) {
      return { error: 'Player not found' };
    }

    const clearedItems = player.inventory.length;
    player.inventory = [];
    ...
    return {
      success: true,
      message: `Inventory cleared, removed ${clearedItems} items`,
      clearedItems
    };
  }
  ...
  async getPlayersStats() {
    const players = this.gameService.getAllPlayers();
    const onlinePlayers = Object.values(players).filter(p => p.socketId !== '');
    ...
    return {
      ...
      playersByChunk: this.getPlayersByChunk(players)
    };
  }
   ...
   */
  @Get('search/by-item')
  async searchPlayersByItem(@Query('item') item: string) {
    const players = this.gameService.getAllPlayers();
    const result = [];

    for (const [telegramId, player] of Object.entries(players)) {
      const itemCount = player.inventory.filter(i => i === item).length;
      if (itemCount > 0) {
        result.push({
          telegramId,
          playerName: `Player_${telegramId}`,
          itemCount,
